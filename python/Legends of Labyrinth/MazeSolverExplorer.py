...
INIT_TURN_TIME = 0.018  # Used to turn 45 degrees -ish
STEPS_FOR_HOME = 6
...
def act():
    ...
    if decision == "straight":
        ...
        # get the last sublist from main list 
        if len(explorer_list) > 0:
            last_decision = explorer_list[-1]
            last_decision.pop()
            if (len(last_decision) == 0):
                explorer_list.pop()
        turn_around()
    elif decision == "stay":
        print("This is the exploration result: ", explorer_list)
        stay_put()
        return True  # Stop
    elif decision == "leftright":
        explorer_list.append(["left", "right"])
        turn_right()
    elif decision == "leftstraight":
        explorer_list.append(["left", "straight"])
        straight()
    elif decision == "rightstraight":
        explorer_list.append(["straight", "right"])
        turn_right()
    elif decision == "leftrightstraight":
        explorer_list.append(["left", "straight", "right"])
        turn_right()
    else:
        unmapped()
